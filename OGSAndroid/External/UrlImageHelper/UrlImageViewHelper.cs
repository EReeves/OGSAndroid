...
namespace OGSAndroid.External.UrlImageHelper
{
    public class UrlImageViewHelper
    {
        ...
        private static void SetUrlDrawable(Context context, ImageView imageView, string url, int defaultResource,
            long cacheDurationMs)
        {
            Drawable d = null;
        ...
        public static void SetUrlDrawable(ImageView imageView, string url, Drawable defaultDrawable,
            IUrlImageViewCallback callback)
        {
        ...
        public static void SetUrlDrawable(ImageView imageView, string url, Drawable defaultDrawable,
            long cacheDurationMs, IUrlImageViewCallback callback)
        {
        ...
        private static void SetUrlDrawable(Context context, ImageView imageView, string url, Drawable defaultDrawable,
            long cacheDurationMs, IUrlImageViewCallback callback)
        {
            Cleanup(context);
            ...
            if (file.Exists)
            {
                try
                {
                    if (cacheDurationMs == CACHE_DURATION_INFINITE ||
                        DateTime.UtcNow < file.LastWriteTimeUtc.AddMilliseconds(cacheDurationMs))
                    {
                    ...
                    Log.Debug(LOGTAG, "File Cache Expired: " + file.Name);
                }
                catch (Exception ex)
                {
                    Log.Debug(LOGTAG, "File Cache Exception " + ex);
                }
            }
            ...
            var downloaderTask = new AnonymousAsyncTask<string, string, BitmapDrawable>(p =>
            {
                ...
                {
                    Log.Debug(LOGTAG, "Download Error: " + ex.ToString());
                    return null;
                }
            }, bd =>
            {
                ...
                {
                    Log.Debug(LOGTAG, "PostExecute Error: " + ex.ToString());
                }
            });

            downloaderTask.Execute(new Object[] {});
        }
    }
}