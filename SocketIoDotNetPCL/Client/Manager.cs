...
namespace Quobject.SocketIoClientDotNet.Client
{
    public class Manager : Emitter
    {
        ...
        private readonly EngineIoClientDotNet.Client.Socket.Options Opts;
        private readonly List<Packet> PacketBuffer;
        private readonly Queue<On.IHandle> Subs;
        ...
        private void OnOpen()
        {
            ...
            sub = Client.On.Create(socket, Engine.EVENT_ERROR, new ListenerImpl(data => { OnError((Exception) data); }));
            Subs.Enqueue(sub);

            sub = Client.On.Create(socket, Engine.EVENT_CLOSE, new ListenerImpl(data => { OnClose((string) data); }));
            Subs.Enqueue(sub);
        }
        ...
        public Socket Socket(string nsp)
        {
            ...
            socket.On(Client.Socket.EVENT_CONNECT, new ListenerImpl(() => { Connected++; }));
            return socket;
        }
